...
class Summary_taxes extends Summary_report
{
    ...
    public function getData(array $inputs): array
    {
        ...
        $subquery_builder->select("name AS name, CONCAT(IFNULL(ROUND(percent, {$decimals}), 0), '%') AS percent, sales.sale_id AS sale_id, {$sale_subtotal} AS subtotal, IFNULL({$db_prefixsales_items_taxes}.item_tax_amount, 0) AS tax, IFNULL({$sale_total}, {$sale_subtotal}) AS total");
