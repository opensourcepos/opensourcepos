...
{
    public const ALLOWED_SUGGESTIONS_COLUMNS            = ['name', 'brand', 'item_number', 'description', 'cost_price', 'unit_price'];
    public const ALLOWED_SUGGESTIONS_COLUMNS_WITH_EMPTY = ['', 'name', 'brand', 'item_number', 'description', 'cost_price', 'unit_price'];
    ...
    public function search(string $search, array $filters, ?int $rows = 0, ?int $limit_from = 0, ?string $sort = 'items.name', ?string $order = 'asc', ?bool $count_only = false)
    {
        ...
        if ($count_only) {
            ...
            $builder->select('MAX(items.brand) AS brand');
     ...
     */
    public function get_multiple_info(array $item_ids, array|int $location_id): ResultInterface
    {
        $format = $this->db->escape(dateformat_mysql());
