<?php
return [
	'config_address' => 'Adresa companiei',
	'config_address_required' => 'Adresa companiei este un câmp obligatoriu.',
	'config_all_set' => 'All file permissions are set correctly!',
	'config_allow_duplicate_barcodes' => 'Permiteți coduri de bare duplicate',
	'config_apostrophe' => 'apostrof',
	'config_backup_button' => 'Backup',
	'config_backup_database' => 'Rezervă de date',
	'config_barcode' => 'Barcode',
	'config_barcode_company' => 'Numele Companiei',
	'config_barcode_configuration' => 'Configurarea codului de bare',
	'config_barcode_content' => 'Conținutul codului de bare',
	'config_barcode_first_row' => 'Rândul 1',
	'config_barcode_font' => 'Font',
	'config_barcode_formats' => 'Formate de intrare',
	'config_barcode_generate_if_empty' => 'Generați dacă este gol.',
	'config_barcode_height' => 'Înălțime (px)',
	'config_barcode_id' => 'Element Id / Nume',
	'config_barcode_info' => 'Informații privind configurația codurilor de bare',
	'config_barcode_layout' => 'Formatul de coduri de bare',
	'config_barcode_name' => 'Nume',
	'config_barcode_number' => 'Cod de bare',
	'config_barcode_number_in_row' => 'Numar în rânduri',
	'config_barcode_page_cellspacing' => 'Afișați spațiul celular în pagină.',
	'config_barcode_page_width' => 'Afișați lățimea paginii',
	'config_barcode_price' => 'Preț',
	'config_barcode_second_row' => 'Rândul 2',
	'config_barcode_third_row' => 'Rândul 3',
	'config_barcode_tooltip' => 'Avertisment: această caracteristică poate determina importul sau crearea elementelor duplicate. Nu utilizați dacă nu doriți coduri de bare duplicat.',
	'config_barcode_type' => 'Tipul codului de bare',
	'config_barcode_width' => 'Lățime (px)',
	'config_bottom' => 'Subsol',
	'config_cash_button' => '',
	'config_cash_button_1' => '',
	'config_cash_button_2' => '',
	'config_cash_button_3' => '',
	'config_cash_button_4' => '',
	'config_cash_button_5' => '',
	'config_cash_button_6' => '',
	'config_cash_decimals' => 'Decimale de numerar',
	'config_cash_decimals_tooltip' => 'Dacă decimalele în numerar și decimalele monetare sunt aceleași, atunci nu va avea loc nicio rotunjire a numerarului.',
	'config_cash_rounding' => 'Rotunjire numerar',
	'config_category_dropdown' => '',
	'config_center' => 'Centru',
	'config_change_apperance_tooltip' => '',
	'config_comma' => 'virgula',
	'config_company' => 'Numele Companiei',
	'config_company_avatar' => '',
	'config_company_change_image' => 'Schimbați imaginea',
	'config_company_logo' => 'Sigla companiei',
	'config_company_remove_image' => 'Eliminați imaginea',
	'config_company_required' => 'Numele companiei este obligatoriu',
	'config_company_select_image' => 'Selectati imaginea',
	'config_company_website_url' => 'Site-ul Web al companie nu este un URL valid (http://...).',
	'config_country_codes' => 'Cod tara',
	'config_country_codes_tooltip' => 'Lista separata prin virgula a codurilor tarilor pentru cautarea adresei.',
	'config_currency_code' => '',
	'config_currency_decimals' => 'Zecimale Valuta',
	'config_currency_symbol' => 'Simbol Valuta',
	'config_current_employee_only' => '',
	'config_customer_reward' => 'Recompensa',
	'config_customer_reward_duplicate' => 'Recompensa trebuie sa fie unica',
	'config_customer_reward_enable' => 'Activatie Recompensare Client',
	'config_customer_reward_invalid_chars' => 'Recompensa nu poate contine '_'',
	'config_customer_reward_required' => 'Recompensa este un camp obligatoriu',
	'config_customer_sales_tax_support' => 'Suport Clienti privind Taxele pe Vanzari',
	'config_date_or_time_format' => 'Filtru Data si Timp',
	'config_datetimeformat' => 'Format Data si Timp',
	'config_decimal_point' => '',
	'config_default_barcode_font_size_number' => '',
	'config_default_barcode_font_size_required' => '',
	'config_default_barcode_height_number' => '',
	'config_default_barcode_height_required' => '',
	'config_default_barcode_num_in_row_number' => '',
	'config_default_barcode_num_in_row_required' => '',
	'config_default_barcode_page_cellspacing_number' => '',
	'config_default_barcode_page_cellspacing_required' => '',
	'config_default_barcode_page_width_number' => '',
	'config_default_barcode_page_width_required' => '',
	'config_default_barcode_width_number' => '',
	'config_default_barcode_width_required' => '',
	'config_default_item_columns' => '',
	'config_default_origin_tax_code' => '',
	'config_default_receivings_discount' => '',
	'config_default_receivings_discount_number' => '',
	'config_default_receivings_discount_required' => '',
	'config_default_sales_discount' => '',
	'config_default_sales_discount_number' => '',
	'config_default_sales_discount_required' => '',
	'config_default_tax_category' => '',
	'config_default_tax_code' => '',
	'config_default_tax_jurisdiction' => '',
	'config_default_tax_name_number' => '',
	'config_default_tax_name_required' => '',
	'config_default_tax_rate' => '',
	'config_default_tax_rate_1' => '',
	'config_default_tax_rate_2' => '',
	'config_default_tax_rate_3' => '',
	'config_default_tax_rate_number' => '',
	'config_default_tax_rate_required' => '',
	'config_derive_sale_quantity' => '',
	'config_derive_sale_quantity_tooltip' => '',
	'config_dinner_table' => '',
	'config_dinner_table_duplicate' => '',
	'config_dinner_table_enable' => '',
	'config_dinner_table_invalid_chars' => '',
	'config_dinner_table_required' => '',
	'config_dot' => '',
	'config_email' => '',
	'config_email_configuration' => '',
	'config_email_mailpath' => '',
	'config_email_protocol' => '',
	'config_email_receipt_check_behaviour' => '',
	'config_email_receipt_check_behaviour_always' => '',
	'config_email_receipt_check_behaviour_last' => '',
	'config_email_receipt_check_behaviour_never' => '',
	'config_email_smtp_crypto' => '',
	'config_email_smtp_host' => '',
	'config_email_smtp_pass' => '',
	'config_email_smtp_port' => '',
	'config_email_smtp_timeout' => '',
	'config_email_smtp_user' => '',
	'config_enable_avatar' => '',
	'config_enable_avatar_tooltip' => '',
	'config_enable_dropdown_tooltip' => '',
	'config_enable_new_look' => '',
	'config_enable_right_bar' => '',
	'config_enable_right_bar_tooltip' => '',
	'config_enforce_privacy' => '',
	'config_enforce_privacy_tooltip' => '',
	'config_fax' => '',
	'config_file_perm' => 'There are problems with file permissions please fix and reload this page.',
	'config_financial_year' => '',
	'config_financial_year_apr' => '',
	'config_financial_year_aug' => '',
	'config_financial_year_dec' => '',
	'config_financial_year_feb' => '',
	'config_financial_year_jan' => '',
	'config_financial_year_jul' => '',
	'config_financial_year_jun' => '',
	'config_financial_year_mar' => '',
	'config_financial_year_may' => '',
	'config_financial_year_nov' => '',
	'config_financial_year_oct' => '',
	'config_financial_year_sep' => '',
	'config_floating_labels' => '',
	'config_gcaptcha_enable' => '',
	'config_gcaptcha_secret_key' => '',
	'config_gcaptcha_secret_key_required' => '',
	'config_gcaptcha_site_key' => '',
	'config_gcaptcha_site_key_required' => '',
	'config_gcaptcha_tooltip' => '',
	'config_general' => '',
	'config_general_configuration' => '',
	'config_giftcard_number' => '',
	'config_giftcard_random' => '',
	'config_giftcard_series' => '',
	'config_image_allowed_file_types' => '',
	'config_image_max_height_tooltip' => '',
	'config_image_max_size_tooltip' => '',
	'config_image_max_width_tooltip' => '',
	'config_image_restrictions' => '',
	'config_include_hsn' => '',
	'config_info' => '',
	'config_info_configuration' => '',
	'config_input_groups' => '',
	'config_integrations' => '',
	'config_integrations_configuration' => '',
	'config_invoice' => '',
	'config_invoice_configuration' => '',
	'config_invoice_default_comments' => '',
	'config_invoice_email_message' => '',
	'config_invoice_enable' => '',
	'config_invoice_printer' => '',
	'config_invoice_type' => '',
	'config_is_readable' => '',
	'config_is_writable' => 'is writable, but the permissions are higher than 750.',
	'config_item_markup' => '',
	'config_jsprintsetup_required' => '',
	'config_language' => '',
	'config_last_used_invoice_number' => '',
	'config_last_used_quote_number' => '',
	'config_last_used_work_order_number' => '',
	'config_left' => '',
	'config_license' => '',
	'config_license_configuration' => '',
	'config_line_sequence' => '',
	'config_lines_per_page' => '',
	'config_lines_per_page_number' => '',
	'config_lines_per_page_required' => '',
	'config_locale' => '',
	'config_locale_configuration' => '',
	'config_locale_info' => '',
	'config_location' => '',
	'config_location_configuration' => '',
	'config_location_info' => '',
	'config_login_form' => '',
	'config_logout' => '',
	'config_mailchimp' => '',
	'config_mailchimp_api_key' => '',
	'config_mailchimp_configuration' => '',
	'config_mailchimp_key_successfully' => '',
	'config_mailchimp_key_unsuccessfully' => '',
	'config_mailchimp_lists' => '',
	'config_mailchimp_tooltip' => '',
	'config_message' => '',
	'config_message_configuration' => '',
	'config_msg_msg' => '',
	'config_msg_msg_placeholder' => '',
	'config_msg_pwd' => '',
	'config_msg_pwd_required' => '',
	'config_msg_src' => '',
	'config_msg_src_required' => '',
	'config_msg_uid' => '',
	'config_msg_uid_required' => '',
	'config_multi_pack_enabled' => '',
	'config_no_risk' => 'No security/vulnerability risks.',
	'config_none' => '',
	'config_notify_alignment' => '',
	'config_number_format' => '',
	'config_number_locale' => '',
	'config_number_locale_invalid' => '',
	'config_number_locale_required' => '',
	'config_number_locale_tooltip' => '',
	'config_os_timezone' => '',
	'config_ospos_info' => '',
	'config_payment_options_order' => '',
	'config_perm_risk' => 'Permissions higher than 750 leaves this software at risk.',
	'config_phone' => '',
	'config_phone_required' => '',
	'config_print_bottom_margin' => '',
	'config_print_bottom_margin_number' => '',
	'config_print_bottom_margin_required' => '',
	'config_print_delay_autoreturn' => '',
	'config_print_delay_autoreturn_number' => '',
	'config_print_delay_autoreturn_required' => '',
	'config_print_footer' => '',
	'config_print_header' => '',
	'config_print_left_margin' => '',
	'config_print_left_margin_number' => '',
	'config_print_left_margin_required' => '',
	'config_print_receipt_check_behaviour' => '',
	'config_print_receipt_check_behaviour_always' => '',
	'config_print_receipt_check_behaviour_last' => '',
	'config_print_receipt_check_behaviour_never' => '',
	'config_print_right_margin' => '',
	'config_print_right_margin_number' => '',
	'config_print_right_margin_required' => '',
	'config_print_silently' => '',
	'config_print_top_margin' => '',
	'config_print_top_margin_number' => '',
	'config_print_top_margin_required' => '',
	'config_quantity_decimals' => '',
	'config_quick_cash_enable' => '',
	'config_quote_default_comments' => '',
	'config_receipt' => '',
	'config_receipt_category' => '',
	'config_receipt_configuration' => '',
	'config_receipt_default' => '',
	'config_receipt_font_size' => '',
	'config_receipt_font_size_number' => '',
	'config_receipt_font_size_required' => '',
	'config_receipt_info' => '',
	'config_receipt_printer' => '',
	'config_receipt_short' => '',
	'config_receipt_show_company_name' => '',
	'config_receipt_show_description' => '',
	'config_receipt_show_serialnumber' => '',
	'config_receipt_show_tax_ind' => '',
	'config_receipt_show_taxes' => '',
	'config_receipt_show_total_discount' => '',
	'config_receipt_template' => '',
	'config_receiving_calculate_average_price' => '',
	'config_recv_invoice_format' => '',
	'config_register_mode_default' => '',
	'config_report_an_issue' => '',
	'config_return_policy_required' => '',
	'config_reward' => '',
	'config_reward_configuration' => '',
	'config_right' => '',
	'config_sales_invoice_format' => '',
	'config_sales_quote_format' => '',
	'config_saved_successfully' => '',
	'config_saved_unsuccessfully' => '',
	'config_security_issue' => 'Security Vulnerability Warning',
	'config_server_notice' => 'Please use the below info for issue reporting.',
	'config_service_charge' => '',
	'config_show_due_enable' => '',
	'config_show_office_group' => '',
	'config_statistics' => '',
	'config_statistics_tooltip' => '',
	'config_stock_location' => '',
	'config_stock_location_duplicate' => '',
	'config_stock_location_invalid_chars' => '',
	'config_stock_location_required' => '',
	'config_suggestions_fifth_column' => '',
	'config_suggestions_first_column' => '',
	'config_suggestions_fourth_column' => '',
	'config_suggestions_layout' => '',
	'config_suggestions_second_column' => '',
	'config_suggestions_third_column' => '',
	'config_system_conf' => 'Setup & Conf',
	'config_system_info' => 'System Info',
	'config_table' => '',
	'config_table_configuration' => '',
	'config_takings_printer' => '',
	'config_tax' => '',
	'config_tax_category' => '',
	'config_tax_category_duplicate' => '',
	'config_tax_category_invalid_chars' => '',
	'config_tax_category_required' => '',
	'config_tax_category_used' => '',
	'config_tax_configuration' => '',
	'config_tax_decimals' => '',
	'config_tax_id' => '',
	'config_tax_included' => '',
	'config_theme' => '',
	'config_theme_preview' => '',
	'config_thousands_separator' => '',
	'config_timezone' => '',
	'config_timezone_error' => '',
	'config_top' => '',
	'config_use_destination_based_tax' => '',
	'config_user_timezone' => '',
	'config_website' => '',
	'config_wholesale_markup' => '',
	'config_work_order_enable' => '',
	'config_work_order_format' => '',
];
