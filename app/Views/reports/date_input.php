...
<?= form_open('#', ['id' => 'item_form', 'enctype' => 'multipart/form-data', 'class' => 'form-horizontal']) ?>
    ...
    <?php if (! empty($mode)) { ?>
        <div class="form-group form-group-sm">
            ...
            <?php } elseif ($mode === 'rmas') { ?>
                <?= form_label(lang('Rmas.rma_type'), 'reports_rma_type_label', ['class' => 'required control-label col-xs-2']) ?>
                <div id="report_rma_type" class="col-xs-3">
                    <?= form_dropdown('rma_type', $rma_type_options, 'all', ['id' => 'input_type', 'class' => 'form-control']) ?>
                </div>
            <?php } ?>
        </div>
    <?php } ?>
