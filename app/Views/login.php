...
<head>
    <meta charset="utf-8">
    <base href="<?= base_url() ?>">
    <title><?= $config['company'] . '&nbsp;|&nbsp;Snapshot&nbsp;|&nbsp;' . lang('Login.login') ?></title>
        ...
        <div
            ...
            <div
                ...
                <?php if (isset($config['company_logo']) && !empty($config['company_logo'])): ?>
                    <img class="logo w-100" src="<?= base_url('uploads/' . $config['company_logo']) ?>"
                ...
                <?php if (empty($config['login_form']) || 'floating_labels' == ($config['login_form'])): ?>
                    <div class="form-floating mt-3">
                        <input class="form-control" id="input-username" name="username" type="text"
                            placeholder="<?= lang('Login.username') ?>" <?php if (ENVIRONMENT == "testing")
                                  echo 'value="admin"'; ?>>
                ...
                <?php elseif ('input_groups' == ($config['login_form'])): ?>
                    <div class="input-group mt-3">
                        ...
                        <input class="form-control" name="username" type="text" placeholder="<?= lang('Login.username'); ?>"
                            aria-label="<?= lang('Login.username') ?>" aria-describedby="input-username" <?php if (ENVIRONMENT == "testing")
                                  echo 'value="admin"'; ?>>
                    </div>
                    <div class="input-group mb-3">
                        ...
                        <input class="form-control" name="password" type="password"
                            ...
                            aria-describedby="input-password" <?php if (ENVIRONMENT == "testing")
                                echo 'value="pointofsale"'; ?>>
                    </div>
                <?php endif; ?>
                <?php
