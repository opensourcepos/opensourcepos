<div id="receipt_wrapper" style="width:100%;">
	<div id="receipt_header" style="text-align:center;">
		...
		?>
			<div id="company_name">
				<img id="image" src="data:image/png;base64,<?php echo base64_encode(file_get_contents('uploads/' . $this->config->get('company_logo'))); ?>" alt="company_logo" />
			</div>
		<?php
		}
		?>
		...
		{
		?>
			<div id="company_name" style="font-size:150%; font-weight:bold;"><?php echo $this->config->get('company'); ?></div>
		<?php
		}
		?>
	...
	<table id="receipt_items" style="text-align:left;width:100%;">
		...
		foreach($cart as $line=>$item)
		{
			...
			?>
				<tr>
					...
					<td style="text-align:right;"><?php echo to_currency($item[($this->config->get('receipt_show_total_discount') ? 'total' : 'discounted_total')]); ?></td>
				</tr>
				<tr>
					<?php
					if($this->config->get('receipt_show_description'))
					{
					?>
					...
					if($this->config->get('receipt_show_serialnumber'))
					{
					?>
		...
		if($this->config->get('receipt_show_total_discount') && $discount > 0)
		{
		?>
			<tr>
		...
		<?php
		if($this->config->get('receipt_show_taxes'))
		{
		?>
			<tr>
				<td colspan="3" style='text-align:right;border-top:2px solid #000000;'><?php echo lang('Sales.sub_total'); ?></td>
		...
		<?php $border = (!$this->config->get('receipt_show_taxes') && !($this->config->get('receipt_show_total_discount') && $discount > 0)); ?>
		<tr>
