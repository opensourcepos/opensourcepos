...
class Sales extends Secure_Controller
{
	...
	public function complete()
	{
		...
		$data['taxes'] = $tax_details[1];
		$data['discount'] = $this->sale_lib->get_discount();
		$data['payments'] = $this->sale_lib->get_payments();

		// Returns 'subtotal', 'total', 'cash_total', 'payment_total', 'amount_due', 'cash_amount_due', 'payments_cover_total'
		$totals = $this->sale_lib->get_totals($tax_details[1]);
		...
		if($this->sale_lib->is_invoice_mode() || $pos_invoice)
		{
			...
			else
			{
				...
				if($invoice_type == 'dynamic' && $customer_info->tax_id != '')
				{
					$invoice_view = 'tax_invoice';
				}
				elseif($invoice_type == 'tax_invoice')
				{
					$invoice_view = 'tax_invoice';
				}
				elseif($invoice_type == 'custom')
				{
					$invoice_view = 'custom_invoice';
				}
				else
				{
	...
	private function _load_sale_data($sale_id)
	{
		...
		if($invoice_type == 'dynamic' && $data['tax_id'] != '')
		{
