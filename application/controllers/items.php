...
class Items extends Secure_area implements iData_controller {
	...
	function index() {
		...
		$data['selected_module'] = 'items';

		$this->smartyci->assign($data);
 		$this->smartyci->display( 'items/manage.php.tpl' );
	}
...
/**END GARRISON ADDED**/
    ...
    foreach($locations_data as $location) {
		...
		$this->smartyci->assign($data);
 		$this->smartyci->display( 'items/form.php.tpl' );
	}
    ...
    $employee_name = array();
    $inventory_array = $this->Inventory->get_inventory_data_for_item($item_id)->result_array();
    ...
    foreach($stock_locations as $location_data) {            
	  ...
	  $data["inventory_array"] = $inventory_array;
		$data["modal_id"]='#'.$this->input->get('modal_id', true);
		$this->smartyci->assign($data);
 		$this->smartyci->display( 'items/inventory.php.tpl' );
    ...
    foreach($stock_locations as $location_data) {            
	  ...
	  $data["inventory_array"] = $inventory_array;
		$data["modal_id"]='#'.$this->input->get('modal_id', true);
		$this->smartyci->assign($data);
 		$this->smartyci->display( 'items/count_details.php.tpl' );
	} //------------------------------------------- Ramel
	...
	function generate_barcodes($item_ids) {
		...
		foreach ($item_ids as $item_id) {
			$item_info = $this->Item->get_info($item_id);
			$result[] = array('name' =>$item_info->name, 'id'=> $item_info->item_number);
		}

		$data['items'] = $result;
		$this->smartyci->assign($data);
 		$this->smartyci->display( 'barcode_sheet.php.tpl' );
	...
	function bulk_edit() {
		...
		$data["modal_id"]='#'.$this->input->get('modal_id', true);
		$this->smartyci->assign($data);
 		$this->smartyci->display( 'items/form_bulk.php.tpl' );
	}
	...
	function excel() {
		...
		$query = $this->Item->get_all_export();
	...
	function excel_import() {
		$data["modal_id"]='#'.$this->input->get('modal_id', true);
		$this->smartyci->assign($data);
 		$this->smartyci->display( 'items/excel_import.php.tpl' );
	}
		...
		if ($_FILES['file_path']['error']!=UPLOAD_ERR_OK) {
            ...
            if ($cols > 24) {
	            for ($col = 24; $col < $cols; $col = $col + 2) {
	                ...
	                if (!is_null($data[$col]) && !is_null($data[$col + 1]))
