...
class Config extends Secure_Controller
{
	...
	private function _licenses()
	{
		...
		$license[$i]['title'] =  $this->lang->line('common_software_title') . '&nbsp;' . $this->config->item('application_version');

		if (file_exists('license/LICENSE')) {
			$license[$i]['text'] = $this->xss_clean(file_get_contents('license/LICENSE'));
		} else {
		...
		foreach ($dir as $fileinfo) {
			...
			if ($fileinfo->isFile()) {
				if ($fileinfo->getExtension() == 'version') {
					...
					$license[$i]['title'] = $this->xss_clean(file_get_contents($basename . '.version'));

					$license_text_file = $basename . '.license';

					if (file_exists($license_text_file)) {
						$license[$i]['text'] = $this->xss_clean(file_get_contents($license_text_file));
					} else {
		...
		// attach the licenses from the LICENSES file generated by NPM
		if ($npm_prod) {
			++$i;
			$license[$i]['title'] = 'Production Dependencies';
			$license[$i]['text'] = '';

			$file = file_get_contents('license/npm_prod.licenses');
		...
		if ($npm_dev) {
			++$i;
			$license[$i]['title'] = 'Development Dependencies';
			$license[$i]['text'] = '';

			$file = file_get_contents('license/npm_dev.licenses');
			$array = json_decode($file, TRUE);
