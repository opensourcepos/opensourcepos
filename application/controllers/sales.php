...
class Sales extends Secure_area
{
    ...
    $cust_info='';
		...
		if ($this->sale_lib->is_invoice_number_enabled() && $this->Sale->invoice_number_exists($invoice_number)) {
			...
			$this->smartyci->display("sales/receipt.php.tpl");
			$this->sale_lib->clear_all();
		}
	...
	function edit($sale_id) {
		...
		$this->smartyci->assign($data);
		$this->smartyci->display("sales/form.php.tpl");
	}
    ...
    $data['stock_location']=$this->sale_lib->get_sale_location();
		...
		$this->smartyci->assign($data);
		$this->smartyci->display('sales/register.php.tpl');
		$this->_remove_duplicate_cookies();
	}
  ...
  function cancel_sale() {
	...
	function suspended() {
		...
		$data["modal_id"]='#'.$this->input->get('modal_id', true);
		$this->smartyci->assign($data);
 		$this->smartyci->display( 'sales/suspended.php.tpl' );
	}
