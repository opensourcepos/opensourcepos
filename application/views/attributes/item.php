<div class="form-group form-group-sm">
	<?= form_label($this->lang->line("attributes_definition_name"), "definition_name_label", array('class' => 'control-label col-xs-3')); ?>
	<div class='col-xs-8'>
		<?= form_dropdown('definition_name', $definition_names, -1, array('id' => 'definition_name', 'class' => 'form-control')); ?>
	</div>
...
<?php foreach ($definition_values as $definition_id => $definition_value) { ?>
	...
	<div class="form-group form-group-sm">
		<?= form_label($definition_value['definition_name'], $definition_value['definition_name'], array('class' => 'control-label col-xs-3')); ?>
		<div class='col-xs-8'>
			<div class="input-group">
				...
				?>
				<span class="input-group-addon input-sm btn btn-outline-secondary remove_attribute_btn"><i class="bi bi-trash"></i></span>
			</div>
		</div>
	</div>
	...
	(function() {
		...
		$("input[name*='attribute_links']").change(function() {
			...
			source: function(request, response) {
				$.get('<?= site_url('attributes/suggest_attribute/'); ?>' + this.element.data('definition-id') + '?term=' + request.term, function(data) {
		...
		var refresh = function() {
			...
			$('#attributes').load('<?= site_url("items/attributes/$item_id"); ?>', {
