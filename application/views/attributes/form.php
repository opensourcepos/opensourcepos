...
<?= form_open('attributes/save_definition/' . $definition_id, array('id' => 'attribute_form', 'class' => 'form-horizontal')); ?>
<fieldset id="attribute_basic_info">

	<div class="form-group form-group-sm">
		<?= form_label($this->lang->line('attributes_definition_name'), 'definition_name', array('class' => 'required control-label col-xs-3')); ?>
		<div class='col-xs-8'>
			<?= form_input(
				array(
	...
	<div class="form-group form-group-sm">
		<?= form_label($this->lang->line('attributes_definition_type'), 'definition_type', array('class' => 'required control-label col-xs-3')); ?>
		<div class='col-xs-8'>
	...
	<div class="form-group form-group-sm">
		<?= form_label($this->lang->line('attributes_definition_group'), 'definition_group', array('class' => 'control-label col-xs-3')); ?>
		<div class='col-xs-8'>
			<?= form_dropdown('definition_group', $definition_group, $definition_info->definition_fk, 'id="definition_group" class="form-control" ' . (empty($definition_group) ? 'disabled="disabled"' : '')); ?>
		</div>
	</div>

	<div class="form-group form-group-sm hidden">
		<?= form_label($this->lang->line('attributes_definition_flags'), 'definition_flags', array('class' => 'control-label col-xs-3')); ?>
		<div class='col-xs-8'>
			<div class="input-group">
				<?= form_multiselect('definition_flags[]', $definition_flags, array_keys($selected_definition_flags), array('id' => 'definition_flags', 'class' => 'selectpicker show-menu-arrow', 'data-none-selected-text' => $this->lang->line('common_none_selected_text'), 'data-selected-text-format' => 'count > 1', 'data-style' => 'btn-outline-secondary', 'data-width' => 'fit')); ?>
			</div>
		</div>
	</div>

	<div class="form-group form-group-sm hidden">
		<?= form_label($this->lang->line('attributes_definition_unit'), 'definition_units', array('class' => 'control-label col-xs-3')); ?>
		<div class='col-xs-8'>
			<div class="input-group">
				<?= form_input(array('name' => 'definition_unit', 'value' => $definition_info->definition_unit, 'class' => 'form-control input-sm', 'id' => 'definition_unit')); ?>
			</div>
		</div>
	</div>

	<div class="form-group form-group-sm hidden">
		<?= form_label($this->lang->line('attributes_definition_values'), 'definition_value', array('class' => 'control-label col-xs-3')); ?>
		<div class='col-xs-8'>
			<div class="input-group">
				<?= form_input(array('name' => 'definition_value', 'class' => 'form-control input-sm', 'id' => 'definition_value')); ?>
				<span id="add_attribute_value" class="input-group-addon input-sm btn btn-outline-secondary">
	...
	<div class="form-group form-group-sm hidden">
		<?= form_label('&nbsp;', 'definition_list_group', array('class' => 'control-label col-xs-3')); ?>
		<div class='col-xs-8'>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		var values = [];
		var definition_id = <?= $definition_id; ?>;
		...
		var remove_attribute_value = function() {
			...
			if (is_new) {
				...
				$.post('<?= site_url($controller_name . "/delete_attribute_value/"); ?>', {
		...
		var add_attribute_value = function(value) {
			...
			if (is_event) {
				...
				if (is_new) {
					...
					$.post('<?= site_url("attributes/save_attribute_value/"); ?>', {
						definition_id: definition_id,
						attribute_value: value
					});
				}
			...
			$('#definition_list_group').append("<li class='list-group-item'>" + value + "<a href='javascript:void(0);'><i class='bi bi-trash'></i></a></li>")
		...
		$('#attribute_form').validate($.extend({
			...
			messages: {
				definition_name: "<?= $this->lang->line('attributes_definition_name_required'); ?>",
				definition_type: "<?= $this->lang->line('attributes_definition_type_required'); ?>"
			}
		}, form_support.error));
	});
</script>