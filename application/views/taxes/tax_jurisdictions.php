<?= form_open('taxes/save_tax_jurisdictions/', array('id' => 'tax_jurisdictions_form', 'class' => 'form-horizontal')); ?>

<?php
$title_jurisdictions['config_title'] = $this->lang->line('taxes_tax_jurisdictions_configuration');
$this->load->view('configs/config_header', $title_jurisdictions);
?>
...
<div class="d-flex justify-content-end">
	<button class="btn btn-primary" name="submit_tax_jurisdictions"><?= $this->lang->line('common_submit'); ?></button>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		var tax_type_options = '<?= $tax_type_options; ?>';
		...
		var add_tax_jurisdiction = function() {
			...
			$(new_block).find('label').html("<?= $this->lang->line('taxes_tax_jurisdiction'); ?> " + tax_jurisdictions_count).attr('for', new_jurisdiction_name_id).attr('class', 'control-label col-xs-2');
		...
		$.validator.addMethod('check4TaxJurisdictionDups', function(value, element) {
			...
			return true;
		}, "<?= $this->lang->line('taxes_tax_jurisdiction_duplicate'); ?>");
		...
		$.validator.addMethod('validateTaxJurisdictionCharacters', function(value, element) {
			...
			return true;
		}, "<?= $this->lang->line('taxes_tax_jurisdiction_invalid_chars'); ?>");
		...
		$.validator.addMethod('requireTaxJurisdiction', function(value, element) {
			...
			return true;
		}, "<?= $this->lang->line('taxes_tax_jurisdiction_required'); ?>");
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					success: function(response) {
						...
						$("#tax_jurisdictions").load('<?= site_url("taxes/ajax_tax_jurisdictions"); ?>', init_add_remove_tax_jurisdiction);
					},
					dataType: 'json'
				});
			},
