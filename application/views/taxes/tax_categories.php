<?= form_open('taxes/save_tax_categories/', array('id' => 'tax_categories_form', 'class' => 'form-horizontal')); ?>

<?php
$title_categories['config_title'] = $this->lang->line('taxes_tax_categories_configuration');
$this->load->view('configs/config_header', $title_categories);
?>
...
<div class="d-flex justify-content-end">
	<button class="btn btn-primary" name="submit_tax_categories"><?= $this->lang->line('common_submit'); ?></button>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		var add_tax_category = function() {
			...
			$(new_block).find('label').html("<?= $this->lang->line('taxes_tax_category'); ?> " + tax_categories_count).attr('for', new_tax_category_id).attr('class', 'control-label col-xs-2');
		...
		$.validator.addMethod("check4TaxCategoryDups", function(value, element) {
			...
			return true;
		}, "<?= $this->lang->line('taxes_tax_category_duplicate'); ?>");
		...
		$.validator.addMethod('validateTaxCategoryCharacters', function(value, element) {
			...
			return true;
		}, "<?= $this->lang->line('taxes_tax_category_invalid_chars'); ?>");
		...
		$.validator.addMethod('requireTaxCategory', function(value, element) {
			...
			return true;
		}, "<?= $this->lang->line('taxes_tax_category_required'); ?>");
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					success: function(response) {
						...
						$("#tax_categories").load('<?= site_url("taxes/ajax_tax_categories"); ?>', init_add_remove_tax_categories);
					},
					dataType: 'json'
				});
			},
