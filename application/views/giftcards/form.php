...
<?= form_open('giftcards/save/' . $giftcard_id, array('id' => 'giftcard_form', 'class' => 'form-horizontal')); ?>
<fieldset id="giftcard_basic_info">
	<div class="form-group form-group-sm">
		<?= form_label($this->lang->line('giftcards_person_id'), 'person_name', array('class' => 'control-label col-xs-3')); ?>
		<div class='col-xs-8'>
			<?= form_input(
				array(
					'name' => 'person_name',
	...
	?>
	<div class="form-group form-group-sm">
		<?= form_label($this->lang->line('giftcards_giftcard_number'), 'giftcard_number', array('class' => 'control-label col-xs-3' . $class)); ?>
		<div class='col-xs-4'>
			<?= form_input(
				array(
	...
	<div class="form-group form-group-sm">
		<?= form_label($this->lang->line('giftcards_card_value'), 'giftcard_amount', array('class' => 'required control-label col-xs-3')); ?>
		<div class='col-xs-4'>
			<div class="input-group input-group-sm">
				<?php if (!currency_side()) : ?>
					<span class="input-group-addon input-sm"><?= $this->config->item('currency_symbol'); ?></span>
				<?php endif; ?>
				<?= form_input(
					array(
				...
				<?php if (currency_side()) : ?>
					<span class="input-group-addon input-sm"><b><?= $this->config->item('currency_symbol'); ?></span>
				<?php endif; ?>
			</div>
		</div>
	</div>
		...
		$('#giftcard_form').validate($.extend({
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					...
					error: function(jqXHR, textStatus, errorThrown) {
						table_support.handle_submit("<?= site_url($controller_name); ?>", {
							message: errorThrown
						});
					},
					dataType: 'json'
			...
			rules: {
				...
				giftcard_amount: {
					required: true,
					remote: {
						url: "<?= site_url($controller_name . '/ajax_check_number_giftcard') ?>",
						type: 'POST',
						data: {
			...
			messages: {
				...
				giftcard_amount: {
					...
					remote: "<?= $this->lang->line('giftcards_value'); ?>"
				}
			}
		}, form_support.error));
	});
