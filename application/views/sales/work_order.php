	...
	<script type="text/javascript">
		$(document).ready(function() {
			var send_email = function() {
				$.get('<?= site_url() . "/sales/send_pdf/" . $sale_id_num . "/work_order"; ?>',
					function(response) {
						$.notify({
...
<div class="print_hide" id="control_buttons" style="text-align:right">
	...
	<?php if (isset($customer_email) && !empty($customer_email)) : ?>
		<a href="javascript:void(0);">
			<div class="btn btn-primary" , id="show_email_button"><?= '<i class="bi bi-envelope pe-1"></i>' . $this->lang->line('sales_send_work_order'); ?></div>
		</a>
	<?php endif; ?>
	<?= anchor("sales", '<i class="bi bi-cart2 pe-1></i>' . $this->lang->line('sales_register'), array('class' => 'btn btn-primary', 'id' => 'show_sales_button')); ?>
	<?= anchor("sales/discard_suspended_sale", '<i class="bi bi-x pe-1"></i>' . $this->lang->line('sales_discard'), array('class' => 'btn btn-danger', 'id' => 'discard_work_order_button')); ?>
</div>
...
<div id="page-wrap">
	...
	<div id="block2">
		...
		<table id="meta">
			<tr>
				<td class="meta-head"><?= $work_order_number_label; ?> </td>
	...
	<table id="items">
		...
		?>
				...
				?>
					<tr class="item-row">
						<td></td>
						<td class="item-name" colspan="4"><?= $item['description']; ?></td>
