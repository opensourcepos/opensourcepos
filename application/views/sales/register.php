...
<div id="register_wrapper">
	...
	<div class="panel-body form-group">
		<ul>
			<li class="pull-left first_li">
				<label class="control-label"><?= $this->lang->line('sales_mode'); ?></label>
			</li>
			<li class="pull-left">
			...
			?>
				<li class="pull-left first_li">
					<label class="control-label"><?= $this->lang->line('sales_table'); ?></label>
				</li>
				<li class="pull-left">
					<?= form_dropdown('dinner_table', $empty_tables, $selected_table, array('onchange' => "$('#mode_form').submit();", 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-outline-secondary', 'data-width' => 'fit')); ?>
				</li>
			<?php
			}
			...
			?>
				<li class="pull-left">
					<label class="control-label"><?= $this->lang->line('sales_stock_location'); ?></label>
				</li>
				<li class="pull-left">
					<?= form_dropdown('stock_location', $stock_locations, $stock_location, array('onchange' => "$('#mode_form').submit();", 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-outline-secondary', 'data-width' => 'fit')); ?>
				</li>
			<?php
			}
			?>
			...
			<li class="pull-right">
				<button class='btn btn-outline-secondary modal-dlg' id='show_suspended_sales_button' data-href="<?= site_url($controller_name . "/suspended"); ?>" title="<?= $this->lang->line('sales_suspended_sales'); ?>">
					<i class="bi bi-justify pe-1"></i><?= $this->lang->line('sales_suspended_sales'); ?>
				</button>
			</li>
			...
			?>
				<li class="pull-right">
					<?= anchor(
						...
						'<i class="bi bi-card-list pe-1"></i>' . $this->lang->line('sales_takings'),
	...
	<div class="panel-body form-group">
		<ul>
			<li class="pull-left first_li">
				<label for="item" class='control-label'><?= $this->lang->line('sales_find_or_scan_item_or_receipt'); ?></label>
			</li>
			<li class="pull-left">
				<?= form_input(array('name' => 'item', 'id' => 'item', 'class' => 'form-control input-sm', 'size' => '50', 'tabindex' => ++$tabindex)); ?>
				<span class="ui-helper-hidden-accessible" role="status"></span>
			</li>
			<li class="pull-right">
				<button id='new_item_button' class='btn btn-primary pull-right modal-dlg' data-btn-new="<?= $this->lang->line('common_new') ?>" data-btn-submit="<?= $this->lang->line('common_submit') ?>" data-href="<?= site_url("items/view"); ?>" title="<?= $this->lang->line($controller_name . '_new_item'); ?>">
					<i class="bi bi-tag pe-1"></i><?= $this->lang->line($controller_name . '_new_item'); ?>
				</button>
			</li>
		</ul>
	</div>
	...
	<table class="sales_table_100" id="register">
		<thead>
			<tr>
				...
				<th style="width: 10%;"><?= $this->lang->line('sales_price'); ?></th>
				<th style="width: 10%;"><?= $this->lang->line('sales_quantity'); ?></th>
				<th style="width: 15%;"><?= $this->lang->line('sales_discount'); ?></th>
				<th style="width: 10%;"><?= $this->lang->line('sales_total'); ?></th>
				<th style="width: 5%; "><?= $this->lang->line('sales_update'); ?></th>
			</tr>
		</thead>
					...
					<tr>
						...
						<td>
							<div class="input-group">
								<?= form_input(array('name' => 'discount', 'class' => 'form-control input-sm', 'value' => $item['discount_type'] ? to_currency_no_money($item['discount']) : to_decimals($item['discount']), 'tabindex' => ++$tabindex, 'onClick' => 'this.select();')); ?>
						...
						<td><a href="javascript:document.getElementById('<?= 'cart_' . $line ?>').submit();" title=<?= $this->lang->line('sales_update') ?>><i class="bi bi-arrow-repeat"></i></a></td>
					</tr>
					<tr>
						<?php
						if ($item['item_type'] == ITEM_TEMP) {
						?>
							<td><?= form_input(array('type' => 'hidden', 'name' => 'item_id', 'value' => $item['item_id'])); ?></td>
							<td style="align: center;" colspan="6">
								<?= form_input(array('name' => 'item_description', 'id' => 'item_description', 'class' => 'form-control input-sm', 'value' => $item['description'], 'tabindex' => ++$tabindex)); ?>
							</td>
							<td> </td>
						<?php
						} else {
...
<div id="overall_sale" class="panel panel-default">
	<div class="panel-body">
		<?= form_open($controller_name . "/select_customer", array('id' => 'select_customer_form', 'class' => 'form-horizontal')); ?>
		<?php
		if (isset($customer)) {
		?>
			<table class="sales_table_100">
				<tr>
					<th style="width: 55%;"><?= $this->lang->line("sales_customer"); ?></th>
					<th style="width: 45%; text-align: right;"><?= anchor('customers/view/' . $customer_id, $customer, array('class' => 'modal-dlg', 'data-btn-submit' => $this->lang->line('common_submit'), 'title' => $this->lang->line('customers_update'))); ?></th>
				</tr>
				<?php
				if (!empty($customer_email)) {
				?>
					<tr>
						<th style="width: 55%;"><?= $this->lang->line("sales_customer_email"); ?></th>
						<th style="width: 45%; text-align: right;"><?= $customer_email; ?></th>
					</tr>
				<?php
				}
				?>
				...
				?>
					<tr>
						<th style="width: 55%;"><?= $this->lang->line("sales_customer_address"); ?></th>
						<th style="width: 45%; text-align: right;"><?= $customer_address; ?></th>
					</tr>
				<?php
				}
				?>
				...
				?>
					<tr>
						<th style="width: 55%;"><?= $this->lang->line("sales_customer_location"); ?></th>
						<th style="width: 45%; text-align: right;"><?= $customer_location; ?></th>
					</tr>
				<?php
				}
				?>
				<tr>
					<th style="width: 55%;"><?= $this->lang->line("sales_customer_discount"); ?></th>
				...
				<?php if ($this->config->item('customer_reward_enable') == TRUE) : ?>
					...
					?>
						<tr>
							<th style="width: 55%;"><?= $this->lang->line("rewards_package"); ?></th>
							<th style="width: 45%; text-align: right;"><?= $customer_rewards['package_name']; ?></th>
						</tr>
						<tr>
							<th style="width: 55%;"><?= $this->lang->line("customers_available_points"); ?></th>
							<th style="width: 45%; text-align: right;"><?= $customer_rewards['points']; ?></th>
						</tr>
					<?php
					}
					?>
				<?php endif; ?>
				<tr>
					<th style="width: 55%;"><?= $this->lang->line("sales_customer_total"); ?></th>
				...
				?>
					<tr>
						<th style="width: 55%;"><?= $this->lang->line("sales_customer_mailchimp_status"); ?></th>
						<th style="width: 45%; text-align: right;"><?= $mailchimp_info['status']; ?></th>
					</tr>
				<?php
				}
				?>
		...
		<table class="sales_table_100" id="sale_totals">
			<tr>
				<th style="width: 55%;"><?= $this->lang->line('sales_quantity_of_items', $item_count); ?></th>
				<th style="width: 45%; text-align: right;"><?= $total_units; ?></th>
			</tr>
			<tr>
				<th style="width: 55%;"><?= $this->lang->line('sales_sub_total'); ?></th>
			...
			<tr>
				<th style="width: 55%; font-size: 150%"><?= $this->lang->line('sales_total'); ?></th>
		...
		?>
			<table class="sales_table_100" id="payment_totals">
				<tr>
					<th style="width: 55%;"><?= $this->lang->line('sales_payments_total'); ?></th>
					<th style="width: 45%; text-align: right;"><?= to_currency($payments_total); ?></th>
				</tr>
				<tr>
					<th style="width: 55%; font-size: 120%"><?= $this->lang->line('sales_amount_due'); ?></th>
			...
			<div id="payment_details">
				...
				?>
					<?= form_open($controller_name . "/add_payment", array('id' => 'add_payment_form', 'class' => 'form-horizontal')); ?>
					<table class="sales_table_100">
						<tr>
							<td><?= $this->lang->line('sales_payment'); ?></td>
							<td>
								<?= form_dropdown('payment_type', $payment_options, $selected_payment_type, array('id' => 'payment_types', 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-outline-secondary', 'data-width' => 'fit', 'disabled' => 'disabled')); ?>
							</td>
						</tr>
						<tr>
							<td><span id="amount_tendered_label"><?= $this->lang->line('sales_amount_tendered'); ?></span></td>
							<td>
								<?= form_input(array('name' => 'amount_tendered', 'id' => 'amount_tendered', 'class' => 'form-control input-sm disabled', 'disabled' => 'disabled', 'value' => '0', 'size' => '5', 'tabindex' => ++$tabindex, 'onClick' => 'this.select();')); ?>
							</td>
						</tr>
					</table>
					<?= form_close(); ?>
					...
					?>
							<div class='btn btn-success pull-right' id='finish_sale_button' tabindex="<?= ++$tabindex; ?>"><i class="bi bi-check pe-1"></i><?= $this->lang->line('sales_complete_sale'); ?></div>
					<?php
						}
					}
					?>
				...
				?>
					...
					<table class="sales_table_100">
						<tr>
							<td><?= $this->lang->line('sales_payment'); ?></td>
							<td>
								<?= form_dropdown('payment_type', $payment_options,  $selected_payment_type, array('id' => 'payment_types', 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-outline-secondary', 'data-width' => 'fit')); ?>
							</td>
						</tr>
						<tr>
							<td><span id="amount_tendered_label"><?= $this->lang->line('sales_amount_tendered'); ?></span></td>
							<td>
								<?= form_input(array('name' => 'amount_tendered', 'id' => 'amount_tendered', 'class' => 'form-control input-sm non-giftcard-input', 'value' => to_currency_no_money($amount_due), 'size' => '5', 'tabindex' => ++$tabindex, 'onClick' => 'this.select();')); ?>
								<?= form_input(array('name' => 'amount_tendered', 'id' => 'amount_tendered', 'class' => 'form-control input-sm giftcard-input', 'disabled' => true, 'value' => to_currency_no_money($amount_due), 'size' => '5', 'tabindex' => ++$tabindex)); ?>
							</td>
						</tr>
					</table>
					<?= form_close(); ?>

					<div class='btn btn-success pull-right' id='add_payment_button' tabindex="<?= ++$tabindex; ?>"><i class="bi bi-credit-card pe-1"></i><?= $this->lang->line('sales_add_payment'); ?></div>
				<?php
				}
				?>
				...
				?>
					<table class="sales_table_100" id="register">
						<thead>
							<tr>
								<th style="width: 10%;"><?= $this->lang->line('common_delete'); ?></th>
								<th style="width: 60%;"><?= $this->lang->line('sales_payment_type'); ?></th>
								<th style="width: 20%;"><?= $this->lang->line('sales_payment_amount'); ?></th>
							</tr>
						</thead>
			...
			<?= form_open($controller_name . "/cancel", array('id' => 'buttons_form')); ?>
			<div class="form-group" id="buttons_sale">
				<div class='btn btn-outline-secondary pull-left' id='suspend_sale_button'><i class="bi bi-justify pe-1"></i><?= $this->lang->line('sales_suspend_sale'); ?></div>
				<?php
				// Only show this part if the payment covers the total
				if (!$pos_mode && isset($customer)) {
				?>
					<div class='btn btn-success' id='finish_invoice_quote_button'><i class="bi bi-check pe-1"></i><?= $mode_label; ?></div>
				<?php
				}
				?>

				<div class='btn btn-danger pull-right' id='cancel_sale_button'><i class="bi bi-x pe-1"></i><?= $this->lang->line('sales_cancel_sale'); ?></div>
			</div>
			<?= form_close(); ?>
				...
				<div class="container-fluid">
					<div class="no-gutter row">
						<div class="form-group form-group-sm">
							<div class="col-xs-12">
								<?= form_label($this->lang->line('common_comments'), 'comments', array('class' => 'control-label', 'id' => 'comment_label', 'for' => 'comment')); ?>
								<?= form_textarea(array('name' => 'comment', 'id' => 'comment', 'class' => 'form-control input-sm', 'value' => $comment, 'rows' => '2')); ?>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="form-group form-group-sm">
							<div class="col-xs-6">
								<label for="sales_print_after_sale" class="control-label checkbox">
									<?= form_checkbox(array('name' => 'sales_print_after_sale', 'id' => 'sales_print_after_sale', 'value' => 1, 'checked' => $print_after_sale)); ?>
									<?= $this->lang->line('sales_print_after_sale') ?>
								</label>
							</div>
							...
							?>
								<div class="col-xs-6">
									<label for="email_receipt" class="control-label checkbox">
										<?= form_checkbox(array('name' => 'email_receipt', 'id' => 'email_receipt', 'value' => 1, 'checked' => $email_receipt)); ?>
										<?= $this->lang->line('sales_email_receipt'); ?>
									</label>
								</div>
							<?php
							}
							...
							?>
								<div class="col-xs-6">
									<label for="price_work_orders" class="control-label checkbox">
										<?= form_checkbox(array('name' => 'price_work_orders', 'id' => 'price_work_orders', 'value' => 1, 'checked' => $price_work_orders)); ?>
										<?= $this->lang->line('sales_include_prices'); ?>
									</label>
								</div>
							<?php
							}
							...
							<div class="form-group form-group-sm">
								<div class="col-xs-6">
									<label for="sales_invoice_number" class="control-label checkbox">
										<?= $this->lang->line('sales_invoice_enable'); ?>
									</label>
								</div>
								...
								<div class="col-xs-6">
									<div class="input-group input-group-sm">
										...
										<?= form_input(array('name' => 'sales_invoice_number', 'id' => 'sales_invoice_number', 'class' => 'form-control input-sm', 'value' => $invoice_number)); ?>
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#item').blur(function() {
			$(this).val("<?= $this->lang->line('sales_start_typing_item_name'); ?>");
		});

		$('#item').autocomplete({
			source: "<?= site_url($controller_name . '/item_search'); ?>",
			minChars: 0,
			autoFocus: false,
			delay: 500,
		...
		var clear_fields = function() {
			if ($(this).val().match("<?= $this->lang->line('sales_start_typing_item_name') . '|' . $this->lang->line('sales_start_typing_customer_name'); ?>")) {
				$(this).val('');
			}
		};
		...
		$('#customer').blur(function() {
			$(this).val("<?= $this->lang->line('sales_start_typing_customer_name'); ?>");
		});
		...
		?>
			$('#sales_invoice_number').keyup(function() {
				$.post("<?= site_url($controller_name . '/set_invoice_number'); ?>", {
		...
		$('#price_work_orders').change(function() {
			$.post("<?= site_url($controller_name . '/set_price_work_orders'); ?>", {
				price_work_orders: $(this).is(':checked')
			});
		});

		$('#email_receipt').change(function() {
			$.post("<?= site_url($controller_name . '/set_email_receipt'); ?>", {
				email_receipt: $(this).is(':checked')
			});
		});

		$('#finish_sale_button').click(function() {
			$('#buttons_form').attr('action', "<?= site_url($controller_name . '/complete'); ?>");
			$('#buttons_form').submit();
		});
	...
	function check_payment_type() {
		...
		} else if (($("#payment_types").val() == "<?= $this->lang->line('sales_cash'); ?>" && cash_mode == '1')) {
