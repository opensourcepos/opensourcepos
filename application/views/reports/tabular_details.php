...
<script type="text/javascript">
	$(document).ready(function() {
		...
		<?php if ($this->config->item('customer_reward_enable') == TRUE && !empty($details_data_rewards)) { ?>
		...
		$('#table')
			...
			.bootstrapTable({
				columns: <?= transform_headers($headers['summary'], TRUE); ?>,
				stickyHeader: true,
				...
				onExpandRow: function(index, row, $detail) {
					$detail.html('<table></table>').find("table").bootstrapTable({
						columns: <?= transform_headers_readonly($headers['details']); ?>,
					...
					<?php if ($this->config->item('customer_reward_enable') == TRUE && !empty($details_data_rewards)) { ?>
						$detail.append('<table></table>').find("table").bootstrapTable({
							columns: <?= transform_headers_readonly($headers['details_rewards']); ?>,
