...
<?= form_open_multipart('customers/do_csv_import/', array('id' => 'csv_form', 'class' => 'form-horizontal')); ?>
<fieldset id="item_basic_info">
	<div class="form-group form-group-sm">
		<div class="col-xs-12">
			<a href="<?= site_url('customers/csv'); ?>"><?= $this->lang->line('common_download_import_template'); ?></a>
		</div>
	</div>
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					success: function(response) {
						dialog_support.hide();
						table_support.handle_submit('<?= site_url('customers'); ?>', response);
					},
					dataType: 'json'
				});
			},
